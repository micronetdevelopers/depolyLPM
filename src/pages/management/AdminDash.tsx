...
export const AdminDash: React.FC = () => {
    ...
    const handleReject = async (user: any) => {
        try {
            await updateStatus({ user_id: user.USER_ID, status: "Inactive" }).unwrap();
    ...
    const handleDeleteConfirm = async () => {
        if (userToDelete) {
            try {
                const response = await deleteUser({ user_id: userToDelete.USER_ID }).unwrap();
        ...
        },
        {
            ...
            render: (value: any, record: any) => (
                <div className="flex items-center space-x-2">
                    ...
                    {record.ACTIVE_STATUS !== 'Delete' && record.ACTIVE_STATUS !== 'Pending' && (
                        <>
            ...
            >
                {selectedUser && (
                    <div className="space-y-6">
                        ...
                        {selectedUser.ACTIVE_STATUS === 'Pending' && (
                        ...
                        {selectedUser.ACTIVE_STATUS !== 'Delete' && selectedUser.ACTIVE_STATUS !== 'Pending' && (
                            <div className="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200">
                                ...
                                {selectedUser.ACTIVE_STATUS !== 'Inactive' && (
                                    <Button
                                        onClick={() => {
